			  ...
			  struct v4l2_requestbuffers *b)
{
	...
	case V4L2_MEMORY_MMAP:
		...
		if (b->count < dev->used_buffers) {
			...
			val = dev->write_position;
			i = do_div(val, dev->used_buffers);
			   ...
			   struct v4l2l_buffer *buf)
{
	...
	spin_lock_bh(&dev->lock);
	val = dev->write_position;
	mod = do_div(val, dev->used_buffers);
	dev->bufpos2index[mod] = buf->buffer.index;
	++dev->write_position;
	dev->reread_count = 0;
...
static int get_capture_buffer(struct file *file)
{
	...
	if (dev->write_position == opener->read_position) {
		...
		++opener->reread_count;
		val = (opener->read_position + dev->used_buffers - 1);
		pos = do_div(val, dev->used_buffers);
	} else {
		opener->reread_count = 0;
		if (dev->write_position >
		    opener->read_position + dev->used_buffers)
			opener->read_position = dev->write_position - 1;
		val = opener->read_position;
		pos = do_div(val, dev->used_buffers);
				   ...
				   size_t count, loff_t *ppos)
{
	...
	val = dev->write_position;
	write_index = do_div(val, dev->used_buffers);
